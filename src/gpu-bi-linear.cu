...
static int INPUTHEIGHT = 128;
static int INPUTWIDTH = 128;
...
__global__ void base_kernel(float *input, float *output, int x, int y) {
    ...
    int num_blocks_per_row = x / 64;

    int block_row = block_id / num_blocks_per_row;
    int block_col = block_id % num_blocks_per_row;
    
    int block_start_idx = block_row * 2 * x + block_col * 64;
    int block_start_idx_out = block_row * 4 * 2 * x + block_col * 128;

    // Base Kernel
    int index = thread_id % 4;
    float top_left = input[block_start_idx + (thread_id / 4) * 2];
    float top_right = input[block_start_idx + (thread_id / 4) * 2 + 1];
    float bottom_left = input[block_start_idx + (thread_id / 4) * 2 + x];
    float bottom_right = input[block_start_idx + (thread_id / 4) * 2 + x + 1];
    ...
    // Top_Left Partial Sums for all 4 rows
    ...
    // Top_Right Partial Sums for all 4 rows
    ...
    // Bottom_Left Partial Sums for all 4 rows
    ...
    // Bottom_Right Partial Sums for all 4 rows
...
void decodeImage(float *inputImageR, float *inputImageG, float *inputImageB){
    int index = 0;
    float *tmpBuffer;
    // READ IMAGE and Init buffers
    const char *fileName = "inputs/2400x1800.jpg";
...
int main(){
    ...
    float ctop_left_coeff[] = {1, 2 / 3.0, 1 / 3.0, 0, 2 / 3.0, 4 / 9.0, 2 / 9.0, 0, 1 / 3.0, 2 / 9.0, 1 / 9.0, 0, 0, 0, 0, 0};
    float ctop_right_coeff[] = {0, 1 / 3.0, 2 / 3.0, 1, 0, 2 / 9.0, 4 / 9.0, 2 / 3.0, 0, 1 / 9.0, 2 / 9.0, 1 / 3.0, 0, 0, 0, 0};
    float cbottom_left_coeff[] = {0, 0, 0, 0, 1 / 3.0, 2 / 9.0, 1 / 9.0, 0, 2 / 3.0, 4 / 9.0, 2 / 9.0, 0, 1, 2 / 3.0, 1 / 3.0, 0};
    float cbottom_right_coeff[] = {0, 0, 0, 0, 0, 1 / 9.0, 2 / 9.0, 1 / 3.0, 0, 2 / 9.0, 4 / 9.0, 2 / 3.0, 0, 1 / 3.0, 2 / 3.0, 1};

    cudaMemcpyToSymbol(gtop_left_coeff, &ctop_left_coeff, 16 * sizeof(float));
    cudaMemcpyToSymbol(gtop_right_coeff, &ctop_right_coeff, 16 * sizeof(float));
    cudaMemcpyToSymbol(gbottom_left_coeff, &cbottom_left_coeff, 16 * sizeof(float));
    cudaMemcpyToSymbol(gbottom_right_coeff, &cbottom_right_coeff, 16 * sizeof(float));
    ...
    int block_num = (INPUTHEIGHT * INPUTWIDTH) / (2 * 64);

    cudaEventRecord(st2);
    base_kernel<<<block_num, 128>>>(dev_in_red, dev_out_red, INPUTWIDTH, INPUTHEIGHT);
    base_kernel<<<block_num, 128>>>(dev_in_green, dev_out_green, INPUTWIDTH, INPUTHEIGHT);
    base_kernel<<<block_num, 128>>>(dev_in_blue, dev_out_blue, INPUTWIDTH, INPUTHEIGHT);
    cudaEventRecord(et2);
